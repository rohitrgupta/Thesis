...
\newcommand{\Gitter}[4]{
    \draw[very thin,color=lightgray] (#1,#3) grid (#2,#4);
}
...
\begin{tikzpicture}[scale=1, samples=100, >=latex]
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    \Gitter{-.1}{11.1}{-2.1}{2.1}
    \Koordinatenkreuz{-.2}{11.3}{-2.2}{2.3}{$\omega t$}{}
    ...
    \draw (7.853981634,0) node[below]{$\frac{5\pi}{2}$};
    \draw (9.424775307,0) node[below]{${3\pi}$};
%    \draw (-4,0) circle (3cm);
%    \KoordinatenkreuzOhneLabelsVerschobenKeinPfeil{-7.2}{-.8}{-3.6}{3.6}{-4}
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

    % voltage
    \draw[color=\FarbeSpannung, very thick] plot[id=voltage, domain=0:11] function{\AmplitudeSpannung*sin(x)} node[right] {$v(t)$};
    % voltage circle
    ...
    %\draw[color=\FarbeWinkelZeichnung!50!black, thick] (\AlphaRad, \SpannungsWert)--(\AlphaRad,\StromWert) node[below=18pt] {};
    % angle in the circle
    ...
    % current
    \draw[color=\FarbeStrom, very thick] plot[id=current, domain=0:11] function{\AmplitudeStrom*sin(x+\PhaseRad)} node[right] {$i(t)$};		
    % current circle
...
% resistor
\ZeigerdiagrammText{60}{0}{1}{0.6}

% inductivity / coil
%\ZeigerdiagrammText{60}{-90}{1}{0.6}
